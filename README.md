# 🧾 BillGets (빌겟츠)
> **"빌 게이츠처럼 자신의 자산관리를 꼼꼼히 하고 싶어하는 사람들을 위한 웹"** <br />
> 영수증을 올리기만 하면, 기록부터 분류까지 간편하게 할 수 있는 웹 애플리케이션

---

## 🙋‍♂️ 팀 구성
|              Backend(조장)               |              Backend(팀원)               |             Frontend(팀원)             |              Frontend(팀원)               |
| :---------------------------------------: | :---------------------------------------: | :------------------------------------: | :---------------------------------------: |
| ![](https://github.com/minjae-083.png)  | ![](https://github.com/SSGDO.png)  | ![](https://github.com/kms13579good-hub.png)  | ![](https://github.com/study-raccoon.png)  |
| [변민재](https://github.com/minjae-083) | [김현영](https://github.com/SSGDO) | [김민승](https://github.com/kms13579good-hub) | [배준성](https://github.com/study-raccoon) |

---

## 📚 목차
- [📋 프로젝트 개요](#-프로젝트-개요)
   - [제안 내용](#제안-내용)
   - [개발 배경](#개발-배경)
   - [기대 효과](#기대-효과)
- [⚙️ 제공 기능](#️-제공-기능)
   - [핵심 기능 목록](#핵심-기능-목록)
   - [웹페이지 구성(정보 구조)](#웹페이지-구성정보-구조)
   - [사용자 시나리오](#사용자-시나리오)
- [🛠️ 기술 스택](#️-기술-스택)
- [📊 달성 결과](#-달성-결과)
- [🔧 개발자 가이드](#-개발자-가이드)
   - [사전 요구 사항](#사전-요구-사항)
   - [프로젝트 구조](#프로젝트-구조)
   - [백엔드 실행](#백엔드-실행)
   - [프론트엔드 실행](#프론트엔드-실행)
   - [데이터베이스 설정](#데이터베이스-설정)
   - [테스트 실행](#테스트-실행)
   - [기여 방법(브랜치 전략)](#기여-방법브랜치-전략)
- [📖 용어 설명 및 참고문헌](#-용어-설명-및-참고문헌)
   - [용어 설명](#용어-설명)
   - [참고문헌 및 자료](#참고문헌-및-자료)
 
---

## 📋 프로젝트 개요
 ### 제안 내용
 > 본 프로젝트는 영수증 이미지에서 자동으로 지출 항목을 인식하고, 이를 기반으로 가계부를 자동으로 생성 및 관리해주는 웹 애플리케이션을 개발하는 것을 목표로 합니다.

### 개발 배경
- 현대인의 소비 지출은 다양하게 발생하지만, 수동 가계부 작성은 번거로움으로 인해 지속이 어렵다는 단점이 있습니다.
- 현금 지출과 같이 카드 연동 지출 내역 관리 프로그램으로는 산출할 수 없는 지출에 대한 관리 기능을 제공할 필요가 있습니다.
- 자동화된 소비 기록을 통해 개인의 재무 관리 접근성을 높일 수 있습니다.

### 기대 효과
|         기대 효과       |           내용        |
| :--------------------:  | :--------------------: |
|편의성 향상             | 영수증 촬영 혹은 이미지 업로드만으로 가계부 자동 작성, 입력 시간 절감 |
|지속성 향상             | 자동화로 인한 가계부 작성·관리 습관 형성 용이 |
|데이터 활용             | 월별·카테고리별 소비 패턴 시각화 및 분석 제공 |

---

## ⚙️ 제공 기능
 ### 핵심 기능 목록
 **1. 영수증 스마트 인식 및 관리(OCR)**
  - 영수증 사진 업로드 : 영수증을 업로드하면 이미지 전처리(OpenCV) 및 텍스트 추출(Regex, OCR) 기술을 통해 핵심 정보를 불러옵니다. 인식 신뢰도가 낮을 때는 Claude API로 파싱을 보완(폴백)합니다.
  - 추출 텍스트 수동 조정 : OCR 인식 오류를 대비하여 추출된 정보(가격, 물품명, 가게명, 날짜 등)를 사용자가 먼저 확인 후 수정할 수 있는 권한을 부여합니다.
  - 지출 내역 수동 작성 : 영수증이 없는 현금 지출 건도 사용자가 직접 금액과 내역을 입력하여 누락없이 관리할 수 있습니다.

**2. 지출 분류 및 검색**
 - 카테고리 자동 분류 : 가게명을 분석하여 식비, 교통 등의 카테고리를 자동으로 분류합니다. 분류 결과는 캐싱되어 같은 가게는 API 재호출 없이 즉시 분류되고, 외부 API 장애 시에는 키워드 규칙으로 대체(폴백)되어 중단 없이 동작합니다.
 - 상세 검색 기능 : 가게명 키워드 검색을 통해 찾고자 하는 내역을 쉽게 모아볼 수 있습니다.

**3. 소비 내역 대시보드**
 - 한눈에 보는 캘린더 : 월별 소비 내역을 달력 형태로 시각화하여 제공합니다.
 - 수치로 볼 수 있는 월별 소비 비교 : 월별 추이 막대그래프와 전월 비교를 통해 선택한 달의 지출 증감을 직관적으로 확인할 수 있습니다.
 - 예산 설정 및 초과 경고 : 카테고리별 월 예산을 설정하면 사용률을 진행바로 표시하고, 예산을 초과하면 경고를 보여줍니다.

**4. 내역 파일 생성 및 공유**
 - 자신이 만든 파일 공유 : 특정 목적의 가계부(ex. 교통비 결제 항목만 선택)를 선택하여 파일로 생성한 후, 타인에게 공유할 수 있습니다.

**5. 은행 CSV 가져오기**
 - 은행에서 내려받은 거래내역 CSV 파일을 업로드하면 지출 내역으로 일괄 등록합니다. 은행마다 다른 컬럼명을 유연하게 인식하며, 이미 등록된 내역(같은 날짜·금액·가게)은 자동으로 걸러 중복 등록을 방지합니다.

**6. 회원가입 및 로그인**
 - 회원가입 기능을 통해 다른 기기에서도 웹 페이지에 저장된 자신의 가계부를 확인할 수 있습니다.

### 부가 기능
> BillGets는 사용자의 편의성을 위해 다음과 같은 기능 확장을 준비 및 고려하고 있습니다.

|         기능명       |           내용        |
| :--------------------:  | :--------------------: |
|AI ChatBot 도입           | 페이지의 우측 하단에 AI ChatBot을 배치하여, 간단한 질문 및 서치에 대한 조언을 받을 수 있습니다. |
|비회원 로그인             | 회원가입을 하지 않고 바로 기능을 사용하고 싶은 사용자들을 위함이며, LocalStorage를 활용하여 브라우저 종료 후에도 데이터를 유지할 수 있게 됩니다.  |

---

 ### 웹페이지 구성(정보 구조)
 **1. 메인 페이지**
 <p align="center"> <img width="1389" height="621" alt="Image" src="https://github.com/user-attachments/assets/5ed3f1cd-7346-4998-860c-0481b090cb6a" /> </p>

 - 로그인
 - 지출 캘린더
 - 영수증 업로드
 - 이번 달 지출 현황
 - 최근 지출 내역

---

**2. 영수증 등록**
<p align="center"> <img width="1418" height="617" alt="Image" src="https://github.com/user-attachments/assets/c9168161-5ac1-461b-b666-6137f317456e" /> </p>

- 영수증 이미지 업로드(jpg·png·webp)
- 지출 내역 수동 작성
- 은행 CSV 업로드

---

**3. 지출 내역**
<p align="center"> <img width="1389" height="621" alt="Image" src="https://github.com/user-attachments/assets/b6ff4afd-0fda-4d04-b20a-1a4363d6fe08" /> </p>

- 검색 기능
- 총 건수·지출·수입
- 내역 목록

---

**4. 분석 & 통계**
<p align="center"> <img width="1389" height="621" alt="Image" src="https://github.com/user-attachments/assets/a8e80b1d-7312-4e07-b2f0-d72951c4bc60" /> </p>

- 수입 및 지출 분석
- 월별 추이 · 요일별 분석 · 전월 비교 · 예산 관리

---

**5. 나만의 파일**
<p align="center"> <img width="1389" height="621" alt="Image" src="https://github.com/user-attachments/assets/44aab7a7-8877-47af-a4f7-f58d6e7fa81b" /> </p>

- 파일 추가 및 내역 선택·수정
- 파일 내보내기(Excel)

 ### 사용자 시나리오
 **배경**
 >김현영(23세, 대학생)씨는 월세 납부를 위해 지출 관리를 해야 하지만, 매번 가계부 쓰는게 귀찮아서 늘 작심삼일로 끝났다. 현금도 자주 쓰는 편이라 카드 앱만으로는 지출 파악이 어렵다.

**1. 첫 방문 및 회원가입**
<p align="center"> <img width="1432" height="443" alt="Image" src="https://github.com/user-attachments/assets/56c1e364-5f30-4f0e-9439-b4ed889a78f4" /> </p>

현영이는 친구의 추천으로 BillGets에 처음 접속한다.
 - 메인페이지에서 우측 상단의 로그인 버튼을 클릭해 로그인 페이지로 이동한다.
 - '회원가입' 버튼을 클릭해 이메일과 비밀번호를 입력하고 계정을 생성한다.
 - 로그인 후 메인페이지로 이동한다.
 - 아직 등록된 내역이 없어 "지출 내역이 없습니다." 안내 문구가 보인다.

---

**2. 영수증 관리**
<p align="center"> <img width="1432" height="676" alt="Image" src="https://github.com/user-attachments/assets/85b9cba5-4d19-4e99-8732-b50a123d2af0" /> </p>

현영이는 편의점에서 빵을 카드로 결제하고, 그 영수증을 들고 있다.
 - 카메라로 영수증 사진을 찍는다.
 - 메인페이지에서 '영수증 등록하기' 버튼을 클릭한다.
 - 영수증 사진 파일을 업로드한다.
 - 자동 인식된 가게명, 물품명, 금액, 날짜가 화면에 표시된다.
 - 현영이는 물품명 하나가 잘못 인식되어 있는걸 발견하고 이를 수정한다.
 - 카테고리가 '식비'로 자동 분류된 것을 확인하고 '저장' 버튼을 클릭한다.
 - "저장되었습니다!" 알림이 화면에 표시되고, 현영이는 메인페이지로 돌아온다.

---
 
**3. 현금 지출 수동 입력**
<p align="center"> <img width="1431" height="477" alt="Image" src="https://github.com/user-attachments/assets/21fc97af-f53c-4fe5-a32a-ebe198062f9b" /> </p>

현영이는 동네 포장마차에서 붕어빵과 어묵을 현금으로 결제했고, 영수증은 받아오지 않았다.
 - 영수증 등록 페이지에서 '수동 작성' 탭을 선택한다.
 - 가게명(OO거리 포장마차), 금액(4,000원), 날짜(2026-04-01), 카테고리(식비)를 직접 입력한다.
 - '저장' 버튼을 클릭한다.

---

**4. 분석&통계에서 지출 파악**
<p align="center"> <img width="1431" height="686" alt="Image" src="https://github.com/user-attachments/assets/10f4684c-2e45-4a18-85fb-260305698ac3" /> </p>

월세 납부일이 다가온 현영이는 이번 달 지출을 되돌아보고자 한다.
 - 메인페이지에서 '지출 캘린더'에 날짜별 지출 금액이 표시된다.
 - 월 지출 현황 파이차트에서 식비가 전체의 52%로 가장 많다는 것을 한눈에 파악한다.
 - 분석&통계 탭으로 이동한다.
 - 막대그래프로 시각화된 월별 추이를 확인하니, 지출이 지난 달에 비해 15,000원 더 많다는 것을 확인한다.

---

**5. 지출 내역 검색**
<p align="center"> <img width="1431" height="686" alt="Image" src="https://github.com/user-attachments/assets/10f4684c-2e45-4a18-85fb-260305698ac3" /> </p>

현영이는 저번 주에 갔던 포장마차에서의 지출 기록을 찾고 싶다.
 - '지출 내역' 탭으로 이동한다.
 - 검색창에 "포장마차"를 입력하자, 관련 내역이 표시된다.
 - 원하는 내역을 찾아 상세 내용을 확인하고, 잘못 입력된 날짜를 수정한다.

---

**6. 파일 생성 및 다운로드**
<p align="center"> <img width="1432" height="636" alt="Image" src="https://github.com/user-attachments/assets/4ed6664c-b3af-43a1-a340-05039fd23047" /> </p>

현영이는 이번 달 식비 내역을 정리해서 부모님께 보내드리고 싶다.
 - '나만의 파일' 탭으로 이동한다.
 - '내역 선택 & 저장' 탭에서 파일 이름(3월 식비)과 설명(많이 먹었습니다.)을 입력한다.
 - 카테고리가 '식비'인 내역만 선택하고 '파일로 저장' 버튼을 클릭한다.
 - '저장된 파일' 탭에서 'Excel' 버튼을 클릭해 파일을 다운로드한 뒤, 부모님께 전송한다.

## 🛠️ 기술 스택
|         구분       |          기술/도구        |
| :--------------------:  | :--------------------: |
|  영수증 인식 및 텍스트 추출  | OpenCV, EasyOCR, Claude API, Regex 파싱 |
|  프론트엔드  |  HTML, CSS, JS, React |
|  백엔드  | FastAPI(Python), Supabase  |
|  카테고리 분류  |  Kakao Local API, 키워드 규칙 |
|  버전 관리  | Git / GitHub  |
|  협업 도구  | Notion, Figma |

## 📊 달성 결과
> 제안 단계에서 세운 성공 기준을 실제로 측정·검증한 결과입니다.

|        평가 항목       |          목표        |          결과        |
| :--------------------:  | :--------------------: | :--------------------: |
|  OCR 인식 정확도·속도  | 신뢰도 0.85 이상, 10초 이내 | 고해상도 영수증 평균 신뢰도 0.846 (저해상도 입력은 0.48 — 입력 화질이 핵심 변수), 응답 시간 10초 이내 충족 |
|  카테고리 분류  | 정확도 80% 이상 | 테스트 샘플 기준 목표 충족, 분류 로직 단위 테스트 15건 통과 |
|  데이터 정합성  | DB와 화면(통계·대시보드) 일치 | 회원가입→거래 등록→통계→내보내기 전 흐름 엔드투엔드 검증 통과 |
|  파일 저장·내보내기  | 선택한 내역을 누락·왜곡 없이 저장·출력 | Excel/CSV 내보내기 검증 완료 (한글 인코딩 처리 포함) |
|  자동 테스트  | — | pytest 7개 모듈 51건 전부 통과 |
|  페이지 로딩 속도  | LCP 3.0초 이내 | 프로덕션 번들 약 70KB(gzip)로 경량화 (LCP 정량 측정은 배포 환경에서 진행 예정) |

## 🔧 개발자 가이드
> 프로젝트를 로컬에서 직접 실행하거나 개발에 참여하려는 분들을 위한 안내입니다.

 ### 사전 요구 사항
|        도구       |        버전        |     용도    |
| :--------------------:  | :--------------------: |  :----------------:    |
|  Python  |  3.12 권장  | 백엔드(FastAPI) 실행 |
|  Node.js  |  18 이상  | 프론트엔드(React + Vite) 실행 |
|  Supabase 계정  |  무료 플랜 가능  | 데이터베이스(PostgreSQL) |

 ### 프로젝트 구조
프론트엔드와 백엔드가 한 저장소에 있는 모노레포 구조입니다.
```text
Bill-Gets/
├─ frontend/                  # React(Vite) SPA
│  ├─ src/api/client.js       # 공통 API 클라이언트(JWT 토큰 자동 첨부)
│  ├─ src/pages/              # 페이지별 컴포넌트(메인·영수증 등록·지출 내역·분석·나만의 파일·로그인)
│  ├─ src/contexts/           # 전역 상태(AuthContext: 인증 / TransactionContext: 거래)
│  └─ .env.example            # 환경 변수 예시
└─ backend/                   # FastAPI 서버
   ├─ main.py                 # 앱 진입점(CORS 설정 + 라우터 등록)
   ├─ app/core/               # 설정(config) · JWT 보안(security) · Supabase 클라이언트
   ├─ app/api/routes/         # auth · receipts · transactions · analytics · files
   ├─ app/services/           # OCR · 카테고리 분류 · 내보내기 · CSV 가져오기
   ├─ app/models/schemas.py   # Pydantic 요청/응답 스키마
   ├─ db/schema.sql           # Supabase 테이블 DDL
   ├─ tests/                  # pytest 단위 테스트
   └─ .env.example            # 환경 변수 예시
```

 ### 백엔드 실행
```bash
cd backend
python -m venv .venv
.venv\Scripts\activate            # macOS/Linux: source .venv/bin/activate
pip install -r requirements.txt
copy .env.example .env            # macOS/Linux: cp .env.example .env → 복사 후 값 채우기
uvicorn main:app --reload         # http://localhost:8000 에서 실행
```

`backend/.env` 환경 변수:
|        변수       |        필수 여부        |     설명    |
| :--------------------:  | :--------------------: |  :----------------:    |
|  SUPABASE_URL / SUPABASE_KEY  |  필수  | Supabase 프로젝트 URL과 anon key. 없으면 DB 연동 기능이 동작하지 않습니다. |
|  JWT_SECRET  |  권장  | 토큰 서명 키. 비워두면 실행할 때마다 임의 키가 생성되어 서버 재시작 시 전체 재로그인이 필요합니다. |
|  KAKAO_API_KEY  |  선택  | 카테고리 분류용 Kakao Local API 키. 없으면 키워드 규칙 폴백만으로 분류합니다. |
|  ANTHROPIC_API_KEY  |  선택  | OCR 신뢰도가 낮을 때 Claude API로 파싱을 보완합니다. 없으면 폴백 없이 동작합니다(토큰 종량 과금 주의). |
|  CORS_ORIGINS  |  선택  | 허용 출처(콤마 구분). 기본값은 `http://localhost:5173` 입니다. |

- 첫 영수증 인식 시 EasyOCR 한국어 모델이 자동 다운로드되어 수 분이 걸릴 수 있습니다.
- 서버 실행 후 `http://localhost:8000/docs` 에서 Swagger 기반 API 문서를 확인할 수 있습니다.

 ### 프론트엔드 실행
```bash
cd frontend
npm install
copy .env.example .env            # macOS/Linux: cp — 백엔드가 기본 주소(localhost:8000)면 수정 불필요
npm run dev                       # http://localhost:5173 에서 실행
```
- 백엔드 주소가 다르면 `.env`의 `VITE_API_BASE_URL` 값을 수정합니다.
- 배포용 빌드: `npm run build` → `dist/` 폴더가 생성됩니다(`npm run preview`로 빌드 결과 확인 가능).

 ### 데이터베이스 설정
1. [Supabase](https://supabase.com/)에서 새 프로젝트를 생성합니다.
2. 대시보드의 SQL Editor에 `backend/db/schema.sql` 내용을 붙여넣어 실행합니다 → `users` · `transactions` · `user_files` 테이블과 인덱스가 생성됩니다.
3. Settings → API 에서 프로젝트 URL과 anon key를 복사해 `backend/.env`에 입력합니다.

 ### 테스트 실행
```bash
cd backend
pytest                            # venv 활성화 상태에서 실행
```
- OCR 파싱 · 카테고리 분류 · 인증 스키마 · 거래 스키마 · CSV 가져오기 · 내보내기 · 보안(JWT) 7개 모듈의 단위 테스트가 실행됩니다. 외부 API나 DB 연결 없이 동작합니다.

 ### 기여 방법(브랜치 전략)
1. 저장소를 포크한 뒤 `main`에서 작업 브랜치를 생성합니다 — `feature/<기능명>` 또는 `fix/<수정내용>`.
2. 커밋 메시지에는 `feat:` `fix:` `docs:` 등의 접두사를 사용합니다.
3. 원본 저장소의 `main`으로 Pull Request를 올리고, 리뷰 후 **Squash merge**로 병합합니다(GitHub Flow).
4. 병합 후에는 로컬 `main`을 원본 저장소와 동기화하고 작업 브랜치를 삭제합니다.

## 📖 용어 설명 및 참고문헌
 ### 용어 설명
|        용어       |         설명        |
| :--------------------:  | :--------------------: |
|OCR (Optical Character Recognition)       | 이미지에서 텍스트를 자동으로 인식하고 추출하는 기술 |
|OpenCV           | 이미지 전처리 및 컴퓨터 비전 처리를 위한 오픈소스 라이브러리 |
|EasyOCR             | 한국어를 포함한 다국어 텍스트 인식을 지원하는 오픈소스 OCR 라이브러리   |
|Regex (정규표현식)        | 문자열의 일정한 패턴을 표현하는 일종의 형식 언어  |
|FastAPI           | Python의 API를 빌드하기 위한 웹 프레임워크 |
|Supabase            | PostgreSQL 기반 오픈소스 데이터베이스 서비스   |
|React        | UI 구축을 위한 JS 라이브러리 |
|CSV (Comma-Separated Values)           | 데이터를 쉼표로 구분하여 저장하는 텍스트 기반 파일 형식   |
|Caching(캐싱)          | 자주 사용하는 데이터나 계산 결과를 캐시(임시 저장소)에 복사 후, 다음 요청 시 캐시에서 빠르게 데이터를 제공하여 시스템 성능을 향상시키는 기술    |
|API         | 서로 다른 소프트웨어 애플리케이션이 서로 소통 및 기능을 공유할 수 있도록 정의된 규칙 |
|Fallback          | 어떤 기능이나 서비스가 정상 작동하지 않을 때, 대비책으로 준비된 대체 수단이나 로직 |
|LCP (Largest Contentful Paint)            | 화면에서 가장 큰 이미지·텍스트 블록이 렌더링되는데 걸리는 시간   |

 ### 참고문헌 및 자료
 - PageSpeed Insights: https://pagespeed.web.dev/
 - Kakao Developers API: https://developers.kakao.com/
 - FastAPI 공식 문서: https://fastapi.tiangolo.com/ko/
 - Claude API: https://platform.claude.com/docs/ko/get-started
 - EasyOCR GitHub: https://github.com/JaidedAI/EasyOCR
 - OpenCV 공식 문서: https://docs.opencv.org
 - Supabase 공식 문서: https://supabase.com/docs
 - React 공식 문서: https://react.dev
